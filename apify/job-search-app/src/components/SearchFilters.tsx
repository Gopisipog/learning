...
export function SearchFilters({ onSearch, isLoading }: SearchFiltersProps) {
  ...
  const [timeRange, setTimeRange] = useState<'1h' | '24h' | '7d' | '6m'>('7d');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const filters: SearchFiltersType = {
      ...
      limit,
      includeAi,
      aiEmploymentTypeFilter: employmentTypes.length > 0 ? employmentTypes : undefined,
    ...
    // Store API token temporarily for the search
    ...
    <form className="search-filters" onSubmit={handleSubmit}>
      ...
      <div className="filter-row">
        <div className="filter-group">
          ...
          <select value={timeRange} onChange={e => setTimeRange(e.target.value as '1h'|'24h'|'7d'|'6m')}>
            {TIME_RANGES.map(tr => (
      ...
      <div className="filter-group checkbox-group">
        ...
        <label><input type="checkbox" checked={hasSalary} onChange={e => setHasSalary(e.target.checked)} /> Salary Required</label>
        <label><input type="checkbox" checked={visaSponsorshipOnly} onChange={e => setVisaSponsorshipOnly(e.target.checked)} /> 🛂 Visa Sponsorship Only</label>
      </div>
